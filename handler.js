...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://i.ibb.co/Qjf1sdk/abyss-profile.png'
                    let ppgp = 'https://i.ibb.co/Qjf1sdk/abyss-profile.png'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('fgmods', '/api/welcome', {
                                ...
                                profile: pp,
                                background: 'https://i.ibb.co/Lnmr4HN/abyss5.png'
                            }, 'apikey')
        ...
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.ibb.co/Lnmr4HN/abyss5.png') 
            if (!text)
